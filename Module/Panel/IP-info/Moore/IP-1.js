...
function cleanIspInfo(isp) {
    return isp.replace(/\s?[,]|\s\-|\.$|\(.*\)|(\b(Hong Kong|Mass internet|Communications?|information|international|Technolog(y|ies)|ESolutions?|Services Limited)\b)\s?|Company|munications?/gi, '')
              .replace(/\s+/g, ' ');
}
